        ...
        <svg width={width} height={height} role="img" aria-label="Spending this month compared with last month">
          <defs>
            <linearGradient id={gradient} x1="0" x2="0" y1="0" y2="1">
              <stop offset="0" stopColor="#14A1A5" stopOpacity="0.25" />
              <stop offset="1" stopColor="#14A1A5" stopOpacity="0" />
            </linearGradient>
          </defs>
          {[0.25, 0.5, 0.75].map((f) => (
          ...
          {current.length > 1 ? (
            <>
              ...
              <path d={path(current)} fill="none" stroke="#14A1A5" strokeWidth={2.5} strokeLinejoin="round" />
            </>
          ) : null}
          {last ? (
            <g>
              <circle cx={x(last.day)} cy={y(last.total)} r={5} fill="#fff" stroke="#14A1A5" strokeWidth={2.5} />
